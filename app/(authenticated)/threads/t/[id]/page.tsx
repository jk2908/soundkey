...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  return (
    <>
      <BodyHeading level={1} className="sr-only">
        Thread{' '}
        {users.length === 1 && users[0].userId === user?.userId
          ? 'with yourself'
          : `with ${users.map(u => u.username).join(', ')}`}
      </BodyHeading>
                  ...
                  <EditableMessage.Root messageId={message.messageId}>
                    ...
                    <SpeechBubble
                      ...
                      className={cn(
                        'flex flex-col gap-2',
                        fromMe
                          ? 'fg-app-fg-inverted bg-app-bg-inverted'
                          : 'bg-keyline/80 text-app-fg'
                      )}>
